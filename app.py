...
JSON_FILES = {
    "قانون ج1": os.path.join(BASE_DIR, "V02_Bylaws_P1.json"),
    "قانون ج2": os.path.join(BASE_DIR, "V02_Bylaws_P2.json"),
    "قانون ج3": os.path.join(BASE_DIR, "V02_Bylaws_P3.json"),
    "قانون ج4": os.path.join(BASE_DIR, "V02_Bylaws_P4.json"),
    "قانون ج5": os.path.join(BASE_DIR, "V02_Bylaws_P5.json"),
    "قانون ج6": os.path.join(BASE_DIR, "V02_Bylaws_P6.json"),
    "قانون ج7": os.path.join(BASE_DIR, "V02_Bylaws_P7.json"),
    "قانون ج8": os.path.join(BASE_DIR, "V02_Bylaws_P8.json"),
}
# =====================================================
...
def load_json(kind):
    ...
    if not os.path.exists(path):
        st.warning(f"ملف JSON غير موجود: {path}")
        return []
...
def load_laws(kind):
    ...
    if not laws:
        st.error(f"الملف غير موجود أو فارغ: {JSON_FILES.get(kind, 'غير معروف')}")
        return []
    try:
        with get_cursor() as cur:
    ...
    with st.sidebar:
        ...
        if search:
            results = [
                ...
                if search.lower() in l["Leg_Name"].lower() or search in (l["Leg_Number"] or "")
            ]
            if results:
